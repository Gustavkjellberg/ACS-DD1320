    ...
    if q.peek():
        if letter.match(q.peek()):
            ...
            if q.peek() == ')':
                ...
                ruta.down, numP = readGroup(q, letter, upperCase, lowerCase, number29, number19, number09, weight)
                if numP:
                    ruta.num = int(numP)
                if q.peek():
                    ruta.next, weight = readFormel(q, letter, upperCase, lowerCase, number29, number19, number09, weight)
    ...
    while q.peek() is not None:
        ...
        if q.peek() == "(":
            ...
            if upperCase.match(q.peek()):
                ...
                ruta.down, num = readGroup(q, letter, upperCase, lowerCase, number29, number19, number09, weight)

                if num:
                    ruta.num = int(num)
                skit, num = readGroup(q, letter, upperCase, lowerCase, number29, number19, number09, weight)
                return ruta, num
            else:
                if q.peek() == ')':
            ...
            if q.peek():
                ...
                if not q.peek() == ")":
                    weight += hashtabell.get(ruta.atom)
                    ruta.next, num = readGroup(q, letter, upperCase, lowerCase, number29, number19, number09, weight)
                    print(q.peek())
